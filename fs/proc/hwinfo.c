...
static int hwinfo_proc_show(struct seq_file *m, void *v)
{
	...
	case 0x0198:
		seq_printf(m, "UFS: 东芝UFS硬盘\n");
		break;
	case 0x01ce:
		seq_printf(m, "UFS: 三星UFS硬盘\n");
		break;
	case 0x01ad:
		seq_printf(m, "UFS: 海力士UFS硬盘\n");
		break;
	default:
		seq_printf(m, "UFS: 未知 %x\n", hw_info.emmc_info);
		break;
	}

	switch (hw_info.ddr_info) {
	case HWINFO_DDRID_SAMSUNG:
		seq_printf(m, "DDR: 三星RAM内存\n"); /* 0000 0001B */
		break;
	case HWINFO_DDRID_HYNIX:
		seq_printf(m, "DDR: 海力士RAM内存\n"); /* 0000 0110B */
		break;
	case HWINFO_DDRID_ELPIDA:
		seq_printf(m, "DDR: 尔必达RAM内存\n"); /* 0000 0011B */
		break;
	case HWINFO_DDRID_NANYA:
		seq_printf(m, "DDR: 南亚RAM内存\n"); /* 0000 0101B */
		break;
	case HWINFO_DDRID_INTEL:
		seq_printf(m, "DDR: 英特尔RAM内存\n"); /* 0000 1110B */
		break;
	default:
		seq_printf(m, "DDR: 未知 %x\n", hw_info.ddr_info);
		break;
	}

	switch (hw_info.touch_info) {
	case 1:
		seq_printf(m, "TOUCH IC: 新思\n");
		break;
	case 2:
		seq_printf(m, "TOUCH IC: 智者\n");
		break;
	case 3:
		seq_printf(m, "TOUCH IC: 敦泰\n");
		break;
	case 4:
		seq_printf(m, "TOUCH IC: 未知\n");
		break;
	case 5:
		seq_printf(m, "TOUCH IC: 联咏\n");
		break;
	default:
		seq_printf(m, "TOUCH IC: 未知 %x\n", hw_info.touch_info);
		break;
	}

	switch (hw_info.tp_maker_info) {
	case 1:
		seq_printf(m, "TP Maker: 比尔电子\n");
		break;
	case 2:
		seq_printf(m, "TP Maker: 朗斯电子\n");
		break;
	case 4:
		seq_printf(m, "TP Maker: 欧菲光电子科技\n");
		break;
	case 8:
		seq_printf(m, "TP Maker: 夏普\n");
		break;
	case 17:
		seq_printf(m, "TP Maker: 深超电子\n");
		break;
	case 18:
		seq_printf(m, "TP Maker: LG电子\n");
		break;
	case 22:
		seq_printf(m, "TP Maker: 天马电子\n");
		break;
	case 24:
		seq_printf(m, "TP Maker: 三星电子科技\n");
		break;
	case 33:
		seq_printf(m, "TP Maker: 未知\n");
		break;
	default:
		seq_printf(m, "TP Maker: 未知 %x\n", hw_info.tp_maker_info);
		break;
	}

	switch (hw_info.panel_info) {
	case 0:
		seq_printf(m, "LCD: 日本视界(JDI PANEL)\n");
		break;
	case 4:
		seq_printf(m, "LCD: LG电子(LG PANEL)\n");
		break;
	default:
		seq_printf(m, "LCD: 未知 %x\n", hw_info.panel_info);
		break;
	}

	switch (hw_info.fp_info) {
	case 0:
		seq_printf(m, "Fingerprint: 汇顶科技(Goodix)\n");
		break;
	case 1:
		seq_printf(m, "Fingerprint: 瑞典指纹(FPC)\n");
		break;
	default:
		seq_printf(m, "Fingerprint: 未知 %x\n", hw_info.fp_info);
		break;
	}
